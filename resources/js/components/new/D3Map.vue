...
const mapID = computed(() => `map_${props.data[0].taxa}`)

const json = computed(() => store.state.maps[selected_map_level.value]);
// const json = computed(() => store.state.maps);

const locations = computed(() => {
    const op = [];
    const group_key = (selected_mode.value === 0) ? "state" : "district"
    const grouped_data = d3.groups(props.data, (o) => o[group_key])
    grouped_data.forEach((gb) => {
        op.push({
            name: gb[0],
            value: gb[1].length,
        })
...
const init_vars = ()  => {
	...
	width.value = window.innerWidth * 0.3179
