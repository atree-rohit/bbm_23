...
.table-image img {
    max-height: 5rem;
    border-radius: 1rem;
    box-shadow: 0.1rem 0.1rem 0.33rem 0.1rem rgba(225, 225, 0, 0.25);
}
...
<template>
    <div class="container-fluid">
        <div class="header d-flex px-2">
            ...
            <div v-if="auth">
                <button
                    class="btn btn-lg btn-success mx-5"
            ...
            <div v-else>
                <a
                    href="https://forms.gle/vyMg69M253TkEFd19"
                    target="”_blank”"
                    class="btn btn-lg btn-success"
                    >Become A District Coordinator</a
                >
            </div>
        </div>

        <div class="map-container">
            <district-coordinators @polygon-clicked="polygonClick" />
            ...
            <table class="table table-primary">
                <thead>
                    <tr>
                        ...
                        />
                        <th class="bg-warning text-center" v-if="auth">
                            Actions
                        </th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    >
                        ...
                        <td
                            v-text="
                                getDistrictName(district_coordinator.district)
                            "
                        />
...
import DistrictCoordinators from "./map/DistrictCoordinators.vue";

export default {
    name: "DistrictCoordinatorsComponent",
    components: {
    ...
    methods: {
        polygonClick(polygon) {
            let district_coordinators = this.all_data.filter(
                (d) =>
                    d.state == this.valueFromLabel(polygon.name) ||
                    d.district == this.valueFromLabel(polygon.name)
            );
        ...
        deleteDistrictCoordinator(id) {
            if (
                confirm(
                    "Are you sure you want to delete this District Coordinator?"
                )
            ) {
