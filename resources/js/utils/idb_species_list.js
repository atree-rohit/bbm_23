const DB_NAME = 'BBMCountsCurrentSpeciesList';
const STORE_NAME = 'CurrentSpeciesList';
...
async function saveSpeciesList(data) {
    try {
        const db = await openDB(STORE_NAME);
        const transaction = db.transaction([STORE_NAME], 'readwrite');
        const store = transaction.objectStore(STORE_NAME);
        const request = store.put({id: 1, data: JSON.stringify(data)});
...
async function clearSpeciesList(){
    try {
        const db = await openDB(STORE_NAME);
        const transaction = db.transaction([STORE_NAME], 'readwrite');
        const store = transaction.objectStore(STORE_NAME);
...
export { saveSpeciesList, getSpeciesList, clearSpeciesList}